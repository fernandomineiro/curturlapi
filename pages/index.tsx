...
export default function Home() {
  ...
  const onFinish = async ({ link }: FormValues) => {
    try {
      ...
      setStatus("error");
      setMessage(
        error.response?.data?.error_description || "Alguma coisa deu errado!"
      );
    }
  };
    ...
    <Layout>
      ...
      <Content className={styles.content}>
        <div className={styles.shortner}>
          <Title level={5}>Copia &amp; e cole sua URL no campo</Title>
          <Form
            form={form}
            onFinish={onFinish}
              ...
              <div className={styles.linkFieldInput}>
                <Form.Item
                  ...
                  rules={[
                    {
                      required: true,
                      message: "Por favor coloque o link correto",
                      type: "url",
                    },
                  ]}
                >
      ...
      <Footer className={styles.footer}>App (Fernando) &copy; 2022</Footer>
    </Layout>
  );
}
